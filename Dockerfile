...
ARG M4B_TOOL_DOWNLOAD_LINK="https://github.com/TaylorZaneKirk/auto-m4b/releases/latest/download/m4b-tool.tar.gz"
RUN echo "---- INSTALL M4B-TOOL ----" && \
	if [ ! -f /tmp/m4b-tool.tar.gz ]; then \
...
#    && M4B_TOOL_PRE_RELEASE_LINK=$(wget -q -O - https://github.com/TaylorZaneKirk//releases/tag/latest | grep -o 'M4B_TOOL_DOWNLOAD_LINK=[^ ]*' | head -1 | cut -d '=' -f 2) \
